...
import org.broulf.admin.keybinds.Keybind;
...
public final class BindsCmd extends Command
{
	...
	private void add(String[] args) throws CmdException
	{
		...
		Feature.WURST.getKeybinds().add(key, commands);
	...
	private void remove(String[] args) throws CmdException
	{
		...
		String commands = Feature.WURST.getKeybinds().getCommands(key);
		if(commands == null)
			throw new CmdError("Nothing to remove.");
		
		Feature.WURST.getKeybinds().remove(key);
	...
	private void list(String[] args) throws CmdException
	{
		...
		List<Keybind> binds = Feature.WURST.getKeybinds().getAllKeybinds();
	...
	{
		Feature.WURST.getKeybinds().removeAll();
		ChatUtils.message("All keybinds removed.");
	}
	
	private void reset()
	{
		Feature.WURST.getKeybinds().setKeybinds(KeybindList.DEFAULT_KEYBINDS);
	...
	private void loadProfile(String[] args) throws CmdException
	{
		...
		try
		{
			Feature.WURST.getKeybinds().loadProfile(name);
	...
	private void saveProfile(String[] args) throws CmdException
	{
		...
		try
		{
			Feature.WURST.getKeybinds().saveProfile(name);
	...
	private void listProfiles(String[] args) throws CmdException
	{
		...
		ArrayList<Path> files = Feature.WURST.getKeybinds().listProfiles();
		int page = parsePage(args);
