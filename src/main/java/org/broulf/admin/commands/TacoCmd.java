	...
	implements GUIRenderListener, UpdateListener
{
	...
	public void call(String[] args) throws CmdException
	{
		...
		if(enabled)
		{
			Feature.EVENTS.add(GUIRenderListener.class, this);
			Feature.EVENTS.add(UpdateListener.class, this);
			
		}else
		{
			Feature.EVENTS.remove(GUIRenderListener.class, this);
	...
	{
		Feature.WURST.getCmdProcessor().process("taco");
	}
	...
	public void onRenderGUI(MatrixStack matrixStack, float partialTicks)
	{
		...
		if(Feature.WURST.getHax().rainbowUiHack.isEnabled())
		{
			float[] acColor = Feature.WURST.getGui().getAcColor();
			GL11.glColor4f(acColor[0], acColor[1], acColor[2], 1);
			
		}else
			GL11.glColor4f(1, 1, 1, 1);
		
		Feature.MC.getTextureManager().bindTexture(tacos[ticks / 8]);
		Window sr = Feature.MC.getWindow();
